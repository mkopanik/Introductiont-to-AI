...
class Distances:
    ...
    def __init__(self, en, en2):
        self.en = min(en, 6)
        self.en2 = min(en2, 6)

    def __repr__(self):
        return f'EN:{self.en:.2f}'
...
class Tank:
    def __init__(self):
        ...
        # proximity
        self.proximity_sensors = ["EN"]
        self.proximity_sensors_handles = [0] * 1

        # get handle to proximity sensors
        err_code, self.proximity_sensors_handles = vrep.simxGetObjectHandle(
            self.clientID, "Proximity_sensor", vrep.simx_opmode_blocking)
        ...
        err_code, detectionState, detectedPoint, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            self.clientID, self.proximity_sensors_handles, vrep.simx_opmode_streaming)
    ...
    def read_distances(self):
        distances = []
        err_code, detectionState, detectedPoint, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            self.clientID, self.proximity_sensors_handles, vrep.simx_opmode_buffer)
